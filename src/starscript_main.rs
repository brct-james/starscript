...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get Ship Cooldowns
    // Travel
    let navigation: Option<shared::Navigation>;
    match _client.ship_navigation_status("GREEN-1".to_string()).await {
        Ok(res) => {
            println!("ok {:#?}", res);
            navigation = Some(res.data.navigation);
        }
        Err(res_err) => {
            println!("err {:?}", res_err);
            navigation = None;
    ...
    if locked_cl.ships.len() > 0
        && locked_cl.ships[0].navigation.is_some()
        && locked_cl.ships[0]
            .navigation
            .as_ref()
            .unwrap()
            .arrived_at
            .is_some()
        && locked_cl.ships[0].navigation.as_ref().unwrap().destination != "X1-OE-PM".to_string()
    {
    ...
    println!("Navigation: {:#?}", locked_cl.ships[0].navigation);
    ...
    if locked_cl.ships.len() > 0
        ...
        && locked_cl.ships[0]
            ...
            .arrived_at
            .is_some()
        && locked_cl.ships[0].navigation.as_ref().unwrap().destination != "X1-OE-A005".to_string()
    {
